...
@ContextConfiguration(locations={"StateControllerTests2-context.xml"})
@WebAppConfiguration
public class StateControllerTests2 {
  @Autowired
  ...
  //       @Autowire the StateDAO as a field in this class
  ...
  //       Query the dao for the newly added State and check its fields
  /*
  @Test
  ...
  */
  ...
  public void test_POST_sets_redirect_stateAddedDO_as_view_and_adds_State_to_flashAttributes() {
    try {
      ...
      String viewName = result.getModelAndView().getViewName();
      assertEquals("redirect:stateAdded.do", viewName);
  ...
  @Test
  public void test_GET_stateAddedDO_sets_resultJP_as_viewl() {
    try {
      MvcResult result = mockMvc.perform(
          get("/stateAdded.do")
            .param("abbr", "MI"))
          .andExpect(status().isOk()).andReturn();
      assertEquals("WEB-INF/result.jsp", result.getModelAndView().getViewName());
    }
    catch (Exception e) {
      e.printStackTrace();
      fail(e.toString());
