...
public class MadLibsApp {
	...
	private void addMadLibInput(Scanner input, StringBuilder storyBuilder, String[] storyFragment) {
		...
		for (int i = 0; i < storyFragment.length; i++) {
			if (storyFragment[i] != null) {
				if (i % 2 == 1) {
					System.out.println("Enter a " + storyFragment[i]);
					storyFragment[i] = input.nextLine();
				}
				storyBuilder.append(storyFragment[i]);
			}
		}
